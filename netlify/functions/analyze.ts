...
function getGeminiClient() {
  ...
  if (!apiKey) {
    throw new Error(
      "GEMINI_API_KEY environment variable is required. Please check your Netlify environment variable settings."
    );
  }
...
// ─── Mock Analysis (當缺乏 API Key 時的防錯降級方案) ──────────────────────────
function generateMockAnalysis(
  ...
  const isEcommerce =
    headersStr.includes("銷售額") ||
    headersStr.includes("退貨率") ||
    headersStr.includes("產品類別");
  const isMarketing =
    headersStr.includes("roas") ||
    headersStr.includes("轉換") ||
    headersStr.includes("曝光");
  const isFeedback =
    headersStr.includes("滿意度") ||
    headersStr.includes("評分") ||
    headersStr.includes("nps");
  ...
  if (isEcommerce) {
    ...
    rows.forEach((row, i) => {
      ...
      if (retRate > 0.05) {
        anomalies.push(
          `第 ${i + 2} 列「${cat}」退貨率達 ${(retRate * 100).toFixed(1)}%，明顯偏高。`
        );
      }
    });
...
- **業務建議**：
  ...
  2. 持續推廣表現最優的「${bestCat}」，適當提撥行銷資源以擴大其在地區市場的銷售領先優勢。`;
    ...
    const spendIdx = headers.findIndex(
      (h) => h.includes("花費") || h.includes("支出") || h.includes("成本")
    );
    const revIdx = headers.findIndex(
      (h) => h.includes("營收") || h.includes("收入") || h.includes("額")
    );
    const clickIdx = headers.findIndex((h) => h.includes("點擊"));
    const channelIdx = headers.findIndex(
      (h) => h.includes("渠道") || h.includes("管道")
    );
    ...
    rows.forEach((row, i) => {
      ...
      if (roas > 15) {
        anomalies.push(
          `第 ${i + 2} 列「${channel}」ROAS 效益異常優異 (${roas.toFixed(1)}x)，應驗證數據之歸因模型。`
        );
      }
    });
...
- **業務建議**：
  ...
  2. 針對高花費但 ROAS 偏低的渠道，建議重新調整受眾定位或素材創意，避免行銷預算效益邊際遞減。`;
    ...
    const npsIdx = headers.findIndex(
      (h) => h.includes("NPS") || h.includes("推薦") || h.includes("評分")
    );
    const feedbackIdx = headers.findIndex(
      (h) => h.includes("反饋") || h.includes("意見") || h.includes("評論")
    );
    ...
    rows.forEach((row, i) => {
      ...
      if (nps < 5) {
        ...
        anomalies.push(
          `客戶第 ${i + 2} 列評評分僅 ${nps} 分偏低，反饋意見為：「${txt}」。`
        );
      }
    });
...
- **業務建議**：
  ...
  report += `\n\n> 💡 **系統提示（模擬分析模式）**\n> 偵測到您尚未在系統中配置有效之 \`GEMINI_API_KEY\`，目前正以本機分析引擎為您提供 **DEMO 數據模擬與精確交叉比對**。若要解鎖完整的 Gemini AI 模型全自動深度商業洞察，請至 Netlify 後台「Site settings > Environment variables」設定面板中，新增一個名為 \`GEMINI_API_KEY\` 的環境變數。`;
  ...
  try {
    if (!event.body) {
      return {
        statusCode: 400,
        headers: corsHeaders,
        body: JSON.stringify({ error: "無效的請求內容" }),
      };
    }

    const { csvData, customInstructions } = JSON.parse(event.body);

    if (!csvData || typeof csvData !== "string") {
      return {
        statusCode: 400,
        headers: corsHeaders,
        body: JSON.stringify({ error: "無效的 CSV 數據內容" }),
      };
    }
    ...
    if (isMockMode) {
      console.warn(
        "⚠️ [SYSTEM WARNING] GEMINI_API_KEY is not configured or is the default placeholder. Running in dynamic Mock/Demo Mode..."
      );
      const mockResult = generateMockAnalysis(csvData, customInstructions);
      return {
        statusCode: 200,
        headers: corsHeaders,
        body: JSON.stringify({ analysisResult: mockResult }),
      };
    }
...
請以 Markdown 格式輸出，所有繁體中文部分必須使用**繁體中文**回覆，不要包含任何額外的問候語或結語。`;
    ...
    return {
      statusCode: 200,
      headers: corsHeaders,
      body: JSON.stringify({ analysisResult: response.text }),
    };
  } catch (error: any) {
    ...
    return {
      ...
      headers: corsHeaders,
      body: JSON.stringify({
        error: error.message || "伺服器內部錯誤，請檢查 API Key 設定",
      }),
    };
  }
};
